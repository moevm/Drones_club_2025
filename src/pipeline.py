...
class Pipeline:
    ...
    def _import_class(self, folder_name, class_name):
        try:
            ...
            logging.error(f"Ошибка при импорте класса {class_name}: {e}")
            raise ValueError(f"Ошибка при импорте класса {class_name}: {e}")
    ...
    def set_env(self, test_dict=None):
        parser = argparse.ArgumentParser(description="This is the runner for drones autopilot")
        ...
        parser.add_argument("--vision_attributes", default=False, type=str2bool, help="Whether to record a video from drone (default: False)", metavar="")
        parser.add_argument("--save_images", default=False, type=str2bool, help="Whether to save frames from drone (default: False)", metavar="")
        parser.add_argument("--save_video", default=False, type=str2bool, help="Whether to save video from drone (default: False)", metavar="")
        ...
        parser.add_argument("--plot", default=True, type=bool, help='Whether example is being run with plots (default: "True")', metavar="")
        parser.add_argument("--save_logs", default=True, type=bool, help='Whether example is being run with saving logs (default: "True")', metavar="")
        parser.add_argument("--test", default=False, type=bool, help='Whether example is being run with test - exit after 5 steps (default: "False")', metavar="")
        parser.add_argument("--env_name", default="AutoAviary", type=str, help="Environment class name (default: 'AutoAviary')", metavar="")
        parser.add_argument("--decision_name", default="Decision", type=str, help="Decision class name (default: 'Decision')", metavar="")
        parser.add_argument("--experiment_name", default="default", type=str, help="Experiment name for loading models", metavar="")
        parser.add_argument("--infinity", default=True, type=str2bool, help="Whether to set infinity simulation (default: True)", metavar="")
        parser.add_argument("--separating_step", default=0.4, type=float, help="drone spawn distance == [drone_i * separating_step]", metavar="")
        parser.add_argument("--config", help="Configuration name in the configuration file config.toml")
        parser.add_argument("--scene", default="house", type=str, help="Scene type: 'house' (default), 'hay', 'city'", metavar="", choices=["house", "hay", "city"])
        ...
        if args.config is not None:
            try:
                ...
                logging.error("ERROR :: In the provided configuration file, the configuration is either missing or corrupted.")
                raise
        ...
        self.models = self.load_models(config_name="objects.yaml", experiment_name=args["experiment_name"])
        ...
        env_args = {
            ...
            "scene_type": args.get("scene", "house"),
        }
    ...
    def run(self):
        try:
            ...
            iterator = itertools.count() if self.infinity else range(0, int(self.duration_sec * self.control_freq_hz))
            ...
            for i in iterator:
                ...
                if self.vision_attributes or self.save_images:
                    ...
                    for j in range(self.num_drones):
                        target_pos, target_rpy, target_vel = self.desig.update_move(
                            iteration=i, obs=obs, drone_serial_number=j
                        )
                        target_poss.append(target_pos)
                        target_rpys.append(target_rpy)
                        target_vels.append(target_vel)
            ...
            try:
                ...
                print(f"Failed to plot graphs or save data. Error: {e2}")
