...
class StateLogger(object):
    ...
    def __init__(self,
        ...
        self.controls = np.zeros((num_drones, 12, duration_sec*self.LOGGING_FREQ_HZ)) #### 12 control targets: pos_x,
                                                                                                             ...
                                                                                                             # ang_vel_y,
                                                                                                             # ang_vel_z
        self.save_folder = self.OUTPUT_FOLDER + f'/logs'
    ...
    def save_as_csv(self):
        ### save to csv observation data
        self.save_observation_as_csv("observation")

        ### save to csv control (target) data
        self.save_target_as_csv("target")
        

    def save_observation_as_csv(self,comment: str=""):
        ...
        This is the same function as save_as_csv but w
        """
        if not hasattr(self, "time_save_csv"):
            self.time_save_csv = datetime.now().strftime("%m.%d.%Y_%H.%M.%S")
            
        csv_dir = os.path.join(self.OUTPUT_FOLDER, "save-flight-"+comment+"-"+self.time_save_csv)
        if not os.path.exists(csv_dir):
            os.makedirs(csv_dir+'/')
        t = np.arange(0, self.timestamps.shape[1]/self.LOGGING_FREQ_HZ, 1/self.LOGGING_FREQ_HZ)
        ### fix bug
        min_length = min(len(t), self.states.shape[2])
        t = t[:min_length]
    ...
    def save_target_as_csv(self, comment: str=""):
        if not hasattr(self, "time_save_csv"):
            self.time_save_csv = datetime.now().strftime("%m.%d.%Y_%H.%M.%S")
        ...
        for i in range(self.NUM_DRONES):
            ### save target x-y-z pos
            with open(csv_dir+"/x"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, self.controls[i, 0, :]])), delimiter=",")
            with open(csv_dir+"/y"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, self.controls[i, 1, :]])), delimiter=",")
            with open(csv_dir+"/z"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, self.controls[i, 2, :]])), delimiter=",")
            #### save target roll-pitch-yaw values
            with open(csv_dir+"/r"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, self.controls[i, 6, :]])), delimiter=",")
            with open(csv_dir+"/p"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, self.controls[i, 7, :]])), delimiter=",")
            with open(csv_dir+"/ya"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, self.controls[i, 8, :]])), delimiter=",")
            #### save taget velocity
            with open(csv_dir+"/vx"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, self.controls[i, 3, :]])), delimiter=",")
            with open(csv_dir+"/vy"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, self.controls[i, 4, :]])), delimiter=",")
            with open(csv_dir+"/vz"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t, self.controls[i, 5, :]])), delimiter=",")

            #### save taget velocity derivative (acceleration)
            # TODO

            #### save taget rpy derivative 
            # TODO
